...
add_table(["구분", "내용"], [
    ...
    ["활용\n도서관 데이터",
     ...
     "청구기호 대조·오배열 판정·도서찾기의 기준 데이터. 대출 중으로 기록된 책이 서가에서 발견되는 상태 "
     "불일치 7권을 실증에서 자동 검출. "
...
add_table(["데이터", "출처", "활용 내용"], [
    ["장서 서지·대출 상태", "실증관(대림도서관)\n장서 데이터", "청구기호·서명·자료실·대출 상태 전 장서 20,944권 — "
...
bullet("자관 시스템 연동(도입 단계): 실증은 장서 데이터 스냅샷 기반이라 대출 상태가 하루 단위로 낡는다 — "
       "도서관이 정식 도입하면 자관 전산(ILS)과 연동해 장서·대출 상태를 실시간 반영한다. 그러면 '대출 중으로 "
       "기록된 책이 서가에서 발견되는' 불일치 검증이 상시 기능이 되고, 장서 목록 갱신도 자동화된다.")
