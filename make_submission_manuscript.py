...
sub("1", "이용자의 문제 — '목록에는 있는데 서가에서 못 찾는 책'")
body("도서관 자료는 청구기호 순서로 배가(排架)될 때에만 찾을 수 있다. 이용자가 책을 만나려면 목록 검색 → "
     "청구기호 해석 → 서가 위치 찾기의 계단을 스스로 올라야 하는데, 청구기호 체계에 익숙하지 않은 이용자"
     "(어린이, 고령층, 도서관 초심자)에게는 이 계단 자체가 장벽이다. 게다가 한 권이 다른 자리에 꽂히는 "
     "순간 그 책은 목록에는 존재하지만 서가에서는 찾을 수 없는 책 — 이용자 입장에서 사실상의 결본이 된다.")
sub("2", "사서의 문제 — 오배열 점검은 상시 수행이 어려운 수작업")
body("대출 후 반납 과정, 이용자가 열람 후 임의로 꽂아 두는 행동, 서가 정리 중의 착오 등으로 오배열은 "
     "도서관 운영에서 상시적으로 발생한다. 본 팀이 실증 대상관(영등포구립 대림도서관) 종합자료실 전 장서 "
     "20,944권을 기준 데이터로 서가를 스캔한 결과, 실제 오배열 4건이 검출되어 육안으로 확인되었고, 이와 "
     "별개로 전산 목록과 서가 실물이 어긋난 사례(목록에 없는 실물, 라벨과 전산의 청구기호 불일치, 타관 자료 "
     "혼입 등) 6건이 발견되었다. 서가와 데이터의 불일치는 예외가 아니라 상시 현상인데, 이를 잡아낼 상시 "
     "도구가 없다.")
