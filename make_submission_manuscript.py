...
para("— 이용자의 도서찾기, 사서의 서가점검: 도서관 데이터 기반 온디바이스 AI 서비스 —", size=13, align=C)
...
add_table(["구분", "내용"], [
    ["기획의도 및\n내용 요약",
     "도서관에서 '책을 찾는 일'은 이용자와 사서 양쪽의 문제다. 이용자는 청구기호 체계가 낯설어 목록에서 찾은 "
     "책 앞까지 가고도 헤매고, 한 권이 잘못 꽂히면 그 책은 '소장 중이지만 없는 책'이 된다. 사서의 오배열 점검은 "
     "청구기호를 한 권씩 눈으로 대조하는 수작업이라 상시 수행이 어렵다. LibAR는 스마트폰 카메라로 서가를 "
     "비추면 AI가 책등 라벨(청구기호)을 읽어 장서 데이터와 대조하는 하나의 엔진으로 두 문제를 함께 푼다 — "
     "이용자에게는 찾는 책 한 권을 일치 확률과 함께 짚어 주는 도서찾기·길안내를, 사서에게는 잘못 꽂힌 책을 "
     "빨간색으로 표시하는 서가 점검과 점검 기록을 제공한다. 설치 없이 브라우저에서 동작하며 전 과정이 기기 "
     "안에서 처리된다(공개 데모 운영 중)."],
    ["활용\n도서관 데이터",
     "① 실증관 장서 데이터 — 사서의사결정지원서비스 솔로몬(solomon.data4library.kr, 도서관 빅데이터·필수 "
     "데이터) 기반, 영등포구립 대림도서관 전 장서 20,944권(청구기호·서명·자료실·대출 상태): 청구기호 대조·"
     "오배열 판정·도서찾기의 기준 데이터. 대출 중으로 기록된 책이 서가에서 발견되는 상태 불일치 7권을 "
     "실증에서 자동 검출. "
     "② 장서 데이터 분석: 장서 구조(복본·저자기호 밀집)와 대출 상태를 결합한 구간별 오배열 위험도 산출 — "
     "위험도와 실제 현장 발견의 일치를 통계로 검증(177개 구간, 발견 9구간 중 7구간이 위험도 상위 50%). "
     "③ 도서관 정보나루 Open API: 발전 단계에서 전국 대출 빅데이터를 오배열 로그와 매쉬업해 위험도 분석을 "
     "고도화하고 전국 확장 시 도서관별 장서 연결의 축으로 활용(Ⅵ장)."],
    ["활용 기술",
    ...
    ["결과물",
     ...
     "1장 16권 확인/약 8초(온디바이스), 도서찾기 자리 추정(순서 추론) 정밀도 100%(273/273 검증), 실제 오배열 "
     "4건 검출(육안 확인), 대출-서가 상태 불일치 7권과 전산-서가 불일치(목록 밖 실물 등) 6건 검출, 전 과정 "
     "온디바이스(개인정보·사진 무전송)."],
    ["기대효과",
     "이용자: '못 찾는 책' 감소, 서가 앞 실시간 길안내로 청구기호를 몰라도 책을 만남. 사서: 서가 한 줄 점검이 "
     "'30초 걷기'로 단축, 오배열 로그 축적. 도서관: RFID 같은 고가 인프라 없이 스마트폰만으로 도입, 정보나루 연계로 전국 어느 도서관에나 "
...
sub("1", "이용자의 문제 — 청구기호는 이용자의 언어가 아니다")
body("도서관 자료는 청구기호 순서로 배가(排架)될 때에만 찾을 수 있다. 그런데 이용자가 목록 검색에서 얻는 "
     "'813.6-김54ㅎ' 같은 청구기호는 분류기호와 저자기호의 조합으로, 이를 해석해 서가 위치로 옮기는 것은 "
     "훈련받은 사서의 기술이지 이용자의 언어가 아니다. 규모가 이를 장벽으로 만든다 — 실증관 종합자료실만 "
     "해도 문학(800번대) 서가에 5,678권, 한국소설(813) 분류 하나에 1,839권이 꽂혀 있어, 청구기호를 읽지 "
     "못하는 이용자가 서명만으로 훑을 수 있는 규모가 아니다. 청구기호를 읽는 이용자라도 같은 분류 안에 "
     "저자기호 앞 두 글자까지 같은 책이 40권씩 몰린 구간에서는 다시 헤맨다. 여기에 한 권이 다른 자리에 "
     "꽂히는 순간 그 책은 목록에는 존재하지만 서가에서는 찾을 수 없는 책 — 이용자 입장에서 사실상의 결본이 "
     "된다. 어린이·고령층·도서관 초심자일수록 이 문턱은 높고, 찾다 포기한 경험은 다음 방문을 막는다.")
sub("2", "사서의 문제 — 오배열 점검은 상시 수행이 어려운 수작업")
body("대출 후 반납 과정, 이용자가 열람 후 임의로 꽂아 두는 행동, 서가 정리 중의 착오 등으로 오배열은 "
     "도서관 운영에서 상시적으로 발생한다. 본 팀은 실증 대상관(영등포구립 대림도서관) 종합자료실 전 장서 "
     "20,944권을 대조 기준 데이터로 삼고 그중 일부 구간을 표본 스캔했는데, 표본 점검만으로도 실제 오배열 "
     "4건이 검출되어 육안으로 확인되었고, 이와 별개로 전산 목록과 서가 실물이 어긋난 사례(목록에 없는 실물, "
     "라벨과 전산의 청구기호 불일치, 타관 자료 혼입 등) 6건이 발견되었다. 일부 구간에서 이만큼 나온다는 것은 "
     "전수 점검이 가능해지면 더 드러난다는 뜻이다 — 서가와 데이터의 불일치는 예외가 아니라 상시 현상인데, "
     "이를 잡아낼 상시 도구가 없다.")
...
bullet("결과적으로 대다수 공공도서관에서 오배열 점검은 '가끔, 몰아서' 하는 작업이 되고, 그 사이 이용자는 "
       "소장 중인 책을 찾지 못한 채 돌아간다 — 이용자의 탐색 문제와 사서의 점검 문제는 같은 뿌리에서 나온다.")
...
body("모든 책등에는 이미 규격화된 청구기호 라벨이 붙어 있고, 도서관은 이미 완전한 장서 데이터를 보유하고 "
     ...
     "읽고 장서 데이터와 대조해 '이 자리에 있어야 할 책인가'를 즉시 판정하는 것이다. 같은 판정을 두 방향으로 "
     "쓰면 이용자에게는 '찾는 책이 어디 있는가'(도서찾기), 사서에게는 '어느 책이 잘못 꽂혔는가'(서가 점검)가 "
     "된다 — 하나의 엔진으로 두 사용자를 함께 섬기는 것이 본 서비스의 골격이다.")
...
bullet("현장 데이터 순환 구조: 사용자가 동의하면 스캔한 라벨 조각(서가 전체 사진 아님)이 자동 수집된다. "
       "실증 7주간 축적된 5,200여 조각은 모든 성능 변경을 판정하는 현장 평가셋과 자리 추정 기능의 재료가 "
       "됐다 — 추가 재학습의 개선 효과는 실험으로 확인되지 않아 채택하지 않았음을 Ⅳ장에 그대로 밝힌다.")
...
add_table(["데이터", "출처", "활용 내용"], [
    ["장서 서지·대출 상태\n(필수 도서관 데이터)", "사서의사결정지원서비스\n솔로몬(solomon.\ndata4library.kr) — "
     "실증관\n(대림도서관) 장서", "청구기호·서명·자료실·대출 상태 전 장서 20,944권 — "
     "인식 결과 대조, 오배열 판정, 도서찾기 검색·길안내의 기준 데이터. 대출 중 기록인데 서가에서 발견되는 "
     "상태 불일치 7권 검출"],
    ["전국 대출 빅데이터", "도서관 정보나루\n(data4library.kr)", "오배열 로그와 매쉬업한 위험도 분석 고도화(누적 "
     "대출 회전율로 프록시 교체)와 전국 확장 시 도서관별 장서 연결(Ⅵ장) — 본 실증 예비 분석은 장서 데이터의 "
     "대출 상태를 프록시로 수행(아래 3항)"],
], widths=[32, 36, 82], size=10)
sub("2", "데이터가 실제로 일한 사례 — 대출·서가 상태 불일치 검출")
body("장서 데이터의 대출 상태와 서가 실물을 대조하는 것만으로도 운영 정보가 나온다. 실증 중 대출 중으로 "
     "기록된 책이 서가에서 스캔으로 발견되는 사례가 누적 7권 확인되었는데, 이는 반납 처리와 배가 사이의 "
     "시차·착오를 데이터로 잡아낸 것이다. "
...
body("이용자 기능 쪽 수치도 같은 데이터로 검증했다. 도서찾기의 자리 역산(순서 추론)은 사람 검증 정답 "
     "273건 전건에서 올바른 자리를 맞혔고(정밀도 100%), 찾는 책에 표시하는 일치 확률도 실측 정밀도에서 "
     "가져온 값이다 — 이용자에게 보여 주는 확률이 장식이 아니라 측정값이라는 뜻이다.")
...
sub("5", "한계의 명시 — 한 컷의 판독 범위는 광학이 결정한다")
body("정직한 한계도 기록한다. 청구기호를 읽으려면 라벨 글줄에 원본 사진 기준 일정 픽셀 이상이 떨어져야 "
     "하며, 12MP 사진에서 이는 서가 1~2단을 근접 촬영할 때 확보된다. 서가 여러 열이 한 화면에 들어오는 "
     "원거리 촬영은 실측에서 30% 수준만 판독되었고(4열×3단 광각 사진, 검출 박스 140여 개 중 43권 확정), "
     "검출 해상도를 두 배로 올리는 실험도 이득이 +7%에 그쳤다 — 남은 격차는 소프트웨어가 아니라 라벨에 "
     "도달하는 픽셀의 양, 곧 광학의 한계다. 서비스는 이 한계를 숨기지 않고 설계로 흡수한다: 촬영 가이드"
     "(한 컷에 서가 1~2단), 사진 점검 시 광학 줌 활용(먼 서가·높은 단을 당겨 촬영), 그리고 찾는 책이 화면 "
     "밖이면 청구기호 순서로 이동 방향을 안내하는 UX가 그 대응이다.")
...
for ref in [
    ...
    "국립중앙도서관. (2026). 사서의사결정지원서비스 솔로몬. https://solomon.data4library.kr",
