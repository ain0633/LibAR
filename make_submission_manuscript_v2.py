...
para("— 이용자의 도서찾기, 사서의 서가점검: 스마트폰 하나로 되는 AI 서비스 —", size=13, align=C)
...
add_table(["구분", "내용"], [
    ["기획의도 및\n내용 요약",
     ...
     "화면에 짚어 주고, 사서에게는 잘못 꽂힌 책을 빨간색으로 표시해 줍니다. 설치 없이 웹 주소만 열면 되고, "
     "촬영 화면은 스마트폰 밖으로 전송되지 않습니다. 실증 도서관에서 검증을 마쳤고 공개 데모를 운영 중입니다."],
    ["활용\n도서관 데이터",
     "① 실증관 장서 데이터(영등포구립 대림도서관, 전 장서 20,944권 — 청구기호·서명·자료실·대출 상태): "
     "책 대조·오배열 판정·도서찾기의 기준. 대출 중으로 기록된 책이 서가에서 발견되는 불일치 7권을 자동으로 "
     "찾아냈습니다. ② 장서 데이터 분석: 책 구성과 대출 상태로 '오배열이 잦을 구간'을 예측하는 위험도 분석 — "
     "실제 문제가 발견된 9개 구간 중 7개를 예측이 맞혔습니다. ③ 도서관 정보나루 Open API: 전국 대출 "
     "빅데이터와 결합해 점검 우선순위 추천으로 발전시킬 계획입니다."],
    ["활용 기술",
...
add_table(["데이터", "출처", "쓰임새"], [
    ...
    ["전국 대출 빅데이터", "도서관 정보나루\n(data4library.kr)", "발전 단계: 오배열 기록과 결합해 '어느 서가부터 "
     "점검해야 하는가'를 추천하는 분석으로 확장(Ⅵ장)"],
...
sub("1", "기대효과")
bullet("이용자 — 청구기호를 몰라도 책을 찾습니다. '목록엔 있는데 서가에 없는 책'이 줄고, 어린이·어르신·"
       "도서관 초심자의 정보 접근 문턱이 낮아집니다.")
