...
<div class="step"><b class="num">4</b><b>📦 학습용 조각 제공 — 처음 한 번만 동의</b> — 첫 스캔 후 이 버튼을 눌러 동의하면
<b>이후 스캔마다 라벨 조각이 팀 공유 드라이브로 자동 업로드</b>됩니다 (버튼이 "🔄 자동 제공 중 — N개 전송됨"으로 바뀜).
zip 저장·드라이브 올리기 손작업 없음. <u>서가 전체 사진은 절대 전송되지 않습니다</u> — 우표 크기 라벨 조각 + 인식된 정답뿐.
그만두려면 버튼을 다시 누르면 됩니다</div>
...
<p>4단계 동의 후 자동 전송되는 조각이 정확히 이 교재의 재료입니다. 지금까지 이 방법으로만 학습을
반복해 인식 모델은 v1→v4, 검출 모델은 v1→v3까지 왔습니다 (수작업 라벨링 0회):</p>
...
<tr><td>동의 후 계속 스캔해줌</td><td>조각이 공유 드라이브에 자동 축적 → 다음 학습(v4)의 데이터셋 — 모델 파일 교체만으로 모든 사용자에게 배포</td></tr>
</table>
<div class="concept"><b>프라이버시 원칙:</b> 기본값은 아무것도 전송하지 않습니다. 자동 제공은
버튼을 눌러 <b>명시적으로 동의한 뒤에만</b> 켜지고(다시 누르면 즉시 중지), 전송되는 것은 사진 전체가 아닌
라벨 조각뿐입니다 — 서지 정보는 원래 공개 데이터이고, 내부 관리 정보나 이용자 모습은 애초에 담기지 않습니다.
수집 경로도 업로드 전용 우편함(공유 드라이브 수신함)이라 열람·유출 경로가 없습니다.</div>

<h2>5. 다음 단계</h2>
<table>
<tr><th>단계</th><th>내용</th><th>담당</th></tr>
<tr><td>현장 테스트</td><td>위 5분 코스 (조각은 동의 후 자동 수집)</td><td>광수쌤 🙏</td></tr>
<tr><td>검출기 v4</td><td>드라이브에 모인 조각 + 909 구간 승격 데이터로 재학습 → 걷기 95% 돌파 목표</td><td>아인 (Colab 준비)</td></tr>
